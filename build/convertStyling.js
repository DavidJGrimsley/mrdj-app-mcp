...
export const ConvertStylingInputSchema = z.object({
    ...
    files: z
        ...
        .optional()
        .describe("Optional in-memory file set to scan/convert. Use this when the MCP server cannot access your project folder on disk. If provided, projectRoot is ignored and apply returns edits instead of writing files."),
    basePath: z
        .string()
        .min(1)
        .optional()
        .describe("Optional base path label used for reporting when 'files' are provided (e.g. repo name). This does not need to exist on disk."),
    ...
    async function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                if (options.excludeDirNames.has(entry.name))
                    continue;
                await walk(fullPath);
            }
...
function removeArrayStringLiteral(source, literal) {
...
function upsertUniwindCssImports(source) {
    let updated = source;
    let changed = false;
...
function convertMetroConfig(source) {
    ...
    if (!hasUniwindRequire && /require\(['\"]uniwind\/metro['\"]\)/.test(updated)) {
        ...
        updated = updated.replace(/const\s*\{\s*([a-zA-Z0-9_$]+)\s*\}\s*=\s*require\(['\"]uniwind\/metro['\"]\);?/, "const { withUniwindConfig } = require('uniwind/metro');");
        if (updated !== before) {
            changed = true;
...
function scanCodeFindings(text) {
    const findings = [];
...
function looksLikeWindowsAbsPath(p) {
    return /^[a-zA-Z]:[\\/]/.test(p);
}
function shouldSkipVirtualPath(rel, excludeDirNames) {
...
export async function runConvertStylingTool(params) {
    ...
    const { apply = false, maxFiles = 5000, includeExtensions, excludeDirNames, mode, files: inputFiles, basePath } = parsed.data;
    const excludeDirs = new Set(excludeDirNames ?? [
        "node_modules",
        ".git",
        "build",
        "dist",
    ...
    const scanExts = new Set((includeExtensions ?? [".ts", ".tsx", ".js", ".jsx", ".css", ".json", ".cjs", ".mjs"]).map((e) => e.startsWith(".") ? e : `.${e}`));
    ...
    if (Array.isArray(inputFiles) && inputFiles.length > 0) {
        ...
        for (const f of limited) {
            ...
            if (!rel)
                continue;
            if (shouldSkipVirtualPath(rel, excludeDirs))
                continue;
            const base = path.posix.basename(rel);
            const ext = path.posix.extname(rel);
            const isConfigCandidate = base === "babel.config.js" ||
        ...
        const findingsByKind = findings.reduce((acc, fnd) => {
    ...
    if (typeof requestedRoot === "string" && process.platform !== "win32" && looksLikeWindowsAbsPath(requestedRoot)) {
        return {
            content: [
                {
                    type: "text",
                    text: `Project root looks like a Windows path (${requestedRoot}), but this MCP server is running on ${process.platform}.\n` +
    ...
    for (const absPath of diskFiles) {
        ...
        if (!isSubPath(projectRoot, absPath) && path.resolve(projectRoot) !== path.resolve(absPath))
            continue;
    ...
    const findingsByKind = findings.reduce((acc, f) => {
    ...
    if (changes.length) {
        ...
        if (changes.length > 100)
            lines.push(`...and ${changes.length - 100} more`);
    }
    else {
