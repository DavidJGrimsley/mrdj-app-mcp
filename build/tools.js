...
function validateProjectInfo(markdown) {
...
function validateProjectStyle(markdown) {
    const requiredSections = [
        "Fonts",
        "Colors"
    ];
...
function buildPleskApiDeploymentChecklist(context) {
    const combined = [context.infoText, context.styleText].filter(Boolean).join("\n\n");
    const lower = combined.toLowerCase();
    const hasText = (pattern) => pattern.test(lower);
    const criticalIssues = [];
    // Detect backend pattern
    let backendPattern = "unknown";
    ...
    const checklist = [
        {
            ...
            status: backendPattern !== "unknown" ? "complete" : "incomplete",
        ...
        },
        {
            ...
            status: backendPattern === "expo-api-routes"
                ? (hasText(/expo\.web\.output.*server|web.*output.*server/i) ? "complete" : "incomplete")
                : "n/a",
        ...
        },
        {
            ...
            status: backendPattern === "expo-api-routes"
                ? (hasText(/\+api\.ts|api route/i) ? "complete" : "incomplete")
                : "n/a",
        ...
        },
        {
            ...
            status: backendPattern === "expo-api-routes"
                ? (hasText(/app\.all\(['"]?\*['"]?/i) ? "complete" : "incomplete")
                : "n/a",
        ...
        },
        {
            ...
            status: backendPattern === "expo-api-routes"
                ? (hasText(/express|expo-server|compression|morgan/i) ? "complete" : "incomplete")
                : "n/a",
        ...
        },
        {
            ...
            status: backendPattern === "expo-api-routes" ? "incomplete" : "n/a",
        ...
        },
        {
            ...
            status: backendPattern === "external-api"
                ? (hasText(/api\.|api subdomain/i) ? "complete" : "incomplete")
                : "n/a",
        ...
        },
        {
            ...
            status: hasText(/domain|hosting|plesk.*domain/i) ? "complete" : "incomplete",
...
export function registerTools(params) {
    ...
    server.registerTool("prepare-plesk-deployment", {
    ...
    server.registerTool("copy-ads-code", {
